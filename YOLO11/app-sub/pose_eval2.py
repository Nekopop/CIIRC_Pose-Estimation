...
plt.rcParams['figure.figsize'] = [15, 8]  # Set default figure size larger
...
def evaluate_distance_estimation(dataset_path):
    ...
    Args:
        dataset_path (str): Path to the folder containing RGB images, depth images, and point cloud data
    """
    ...
    if distance_errors:
        # Filter distances up to 12m
        filtered_errors = {d: e for d, e in distance_errors.items() if d <= 12.0}
        
        # Create subplots with larger figure size
        n_plots = len(filtered_errors)
        cols = 2
        rows = (n_plots + 1) // 2
        fig = plt.figure(figsize=(20, 6*rows))  # Set larger size

        for idx, (distance, errors) in enumerate(sorted(filtered_errors.items())):
            ax = plt.subplot(rows, cols, idx+1)
            ax.hist(errors, bins=15, color='skyblue', edgecolor='black')
            ax.set_title(f'Distance: {distance:.1f}m', fontsize=12)
        ...
        plt.tight_layout(pad=3.0)  # Increase padding for more space
        plt.show()

        # Print statistics for each distance range
        print("\nEvaluation Results by Distance:")
        for distance, errors in sorted(distance_errors.items()):
            mean = np.mean(errors)
            std = np.std(errors)
            print(f"Distance {distance:.1f}m - Mean error: {mean:.3f}m, Std: {std:.3f}m")

if __name__ == "__main__":
    evaluate_distance_estimation("dataset/pointscloud/1206/rotated/output-pointclouds_rosbag2_2024_03_07-15_03_23_0")